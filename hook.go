...
func NewHook(proxyURL string, token string,  options *Options) (*Hook, error) {
...
func (h *Hook) flushEntries(entries []*logrus.Entry) error {
	...
	for _, entry := range entries {
		...
		batch.Records = append(batch.Records, json.RawMessage(message))
	}
	...
	if response.StatusCode != http.StatusOK {
		...
		return errors.New(fmt.Sprintf("Unable to deliver payload to REST Proxy on  '%s' HTTP:%d",
			h.ProxyURL, response.StatusCode))
	}
